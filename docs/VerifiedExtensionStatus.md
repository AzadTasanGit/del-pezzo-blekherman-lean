...
| Equation (1) cancels exactly to the Artinian numerator, whose Hilbert function is `(1,c,1,0,...)`, and its degree-two value supplies the quotient socle dimension in the strongest graded rank endpoint | `DelPezzoBlekherman.delPezzoHilbertSeries_mul_one_sub_pow`, `DelPezzoBlekherman.ArtinianReductionDenominatorRelation.toHilbertSeriesCertificate`, `ArtinianGorensteinDegreeOneCertificate.ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate`, `ArtinianGorensteinDegreeOneCertificate.hankelKernel_eq_parameterSpan_and_rank_of_artinianReductionDenominatorRelation` | proved; deriving the exact denominator relation and socle equivalence from the actual regular sequence, plus the Gorenstein zero-annihilator property, remain explicit upstream inputs |
...
`HVectorOneCOneFreeCertificate` and `ArtinianReductionDenominatorRelation` are explicit
...
| 3. `dim W_ell=m+1`, `rank Q_ell=c` | paper-faithful Artinian-reduction endpoint proved conditionally | construct the independent parameter tuple, ambient Hilbert certificate, regular-sequence denominator relation, reduction-to-quotient socle equivalence, and zero-annihilator property from the PDF's AG/CM hypotheses; the reduction certificate, `(1,c,1)` socle dimension, and all subsequent quotient/Hankel steps are internal |
