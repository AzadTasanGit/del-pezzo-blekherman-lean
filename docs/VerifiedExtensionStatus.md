...
The rank endpoint no longer exposes `SuccessiveLinearReductionComponentsRelation`, a reduction
exact-sequence certificate, a Hilbert certificate, or a parameter-product certificate. Its
paper-facing input is the literal Hilbert equation together with
`IsArithmeticallyGorenstein`, the operational regular Artinian reduction and perfect socle
multiplication predicate. `HVectorOneCOneFreeCertificate` remains only at the finite-Proj/degree
boundary and must be removed there. Current mathlib has no packaged graded Hilbert-series,
Cohen--Macaulay, or graded Gorenstein API, which is why the native boundary states the regular
reduction consequences explicitly rather than pretending to derive them from a nonexistent
library predicate.
...
the already checked closed-cone theorem's normalized-square/Zariski-density hypotheses, the
concrete extreme-ray dichotomy, the real-topological projective evaluation map, and the
...
The authoritative order of remaining work is `docs/CompletionPlan.md`.
