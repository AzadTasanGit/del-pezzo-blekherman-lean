...
| 3. `dim W_ell=m+1`, `rank Q_ell=c` | paper-faithful successive-reduction endpoint proved conditionally | construct the independent parameter tuple, ambient Hilbert certificate, one-step finrank recurrences from the regular-sequence exact sequences, reduction-to-quotient socle equivalence, and zero-annihilator property from the PDF's AG/CM hypotheses; their iteration, the denominator/reduction certificates, `(1,c,1)` socle dimension, and all subsequent quotient/Hankel steps are internal |
