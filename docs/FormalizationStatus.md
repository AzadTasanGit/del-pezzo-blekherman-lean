...
|---|---|---|---|---|
| Proposition 2.1 (Hilbert-series consequences) | `delPezzoHilbertSeries`, its coefficient theorems, `degreeOneParameterIdeal_finrank_eq_delPezzoHilbertNumerator`, and `degreeOneParameterIdeal_quotient_finrank` | exact formal-power-series coefficients and the actual regular Artinian reduction's Hilbert function `(1,c,1,0,...)` and total dimension `c+2` proved | the final coordinate-ring setup must supply the literal component Hilbert equation; canonical-module infrastructure is not needed for this numerical bridge | none |
| Proposition 2.2 (regular reductions) | `IsArithmeticallyGorenstein`, `hankelKernel_eq_parameterSpan_and_rank_of_arithmeticallyGorenstein`, `degreeOneParameterIdeal_quotient_finrank`, and `genericRank_eq_add_two_of_flat` | the paper-facing predicate names the actual regular sequence and perfect socle multiplication; the concrete quotient sequences give length `c+2`, the Hankel theorem gives rank `c`, and the parameter extension has generic rank `c+2` under flatness, all without old algebra certificates | instantiate this operational predicate for the coordinate ring and derive flatness over the polynomial hsop subring from the CM/AG hypotheses | none; flatness is the single explicit mathematical bridge still exposed by the generic-rank theorem |
| Proposition 3.1 (closed SOS cone and dual) | `SOSConeDual.sosCone_isClosed_of_separating_multiplicative_evaluations`, `SOSConeDual.gradedSOSCone_isClosed_of_separating_evaluations`, `SOSConeDual.sosCone_eq_finiteSumCone`, `finiteSumCone_closed_fullDimensional_dual_of_separating_evaluations`, and supporting Gram results | the named SOS convex cone is identified with finite sums and is closed directly from compatible point evaluations separating degree one; the coordinate basis, Gram map, and kernel argument are internal, and a graded-algebra adapter constructs multiplication/evaluation; convexity, nonempty interior, and the exact dual identity are also proved | derive separation for the paper's homogeneous coordinate ring from its Zariski-dense real locus and apply the adapter | none; no public Gram certificate remains |
...
| Corollary 3.3 / Lemma 4.1 (evaluation rays and dichotomy) | `theorem1_1_ii_pointEvaluation`, `theorem1_1_ii_dichotomy`, `extreme_hankel_no_complex_basepoint_of_surjective_evaluation` | concrete real evaluation extremality/dichotomy and the paper's indefinite-form exclusion of real-rank-two complex basepoints proved | prove that nonreal projective points give surjective complex degree-one evaluation and rank-one images are real points | none |
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpan_and_rank_of_successiveLinearReductions` | compiling with dependency | for an internal multiplicative grading `𝒜`, Lean constructs `R₁ × R₁ → R₂`; surjectivity of `Sym²(R₁) → R₂` proves `R₁ * R₁ = R₂`, while an equation-(1) Hilbert-series certificate derives both finite-dimensionality and `dim R₁=m+c+1`. The strongest endpoint accepts the local component-finrank recurrence for each of the `(m+1)` linear reductions, iterates it to the denominator identity and `(1,c,1)` certificate, and derives the quotient socle dimension after a degree-two equivalence; only the parameter construction, recurrence proofs, socle equivalence, zero-annihilator proof, and ambient Hilbert certificate must still be obtained from the PDF's basepoint-free AG/CM hypotheses | none; additional hypotheses are explicit |
| Proposition 5.1 | `vanishes_on_ker_iff_eq_smul`, `exists_projective_evaluation_omitting_hyperplane`, `projectiveAevalOfRadical_isFinite_surjective_of_standardGraded_hsop`, `genericRank_eq_add_two_of_flat`, `dense_compl_of_subset_mvPolynomial_zeroSet`, `surjective_of_dense_fiber_pair_bounds` | common-zero/projective-fiber linear algebra, certificate-free finite surjectivity, generic degree `c+2` under flatness, density, compact-image reduction, and perturbation logic proved | derive flatness, connect the native kernel to the scheme morphism, realize continuity, and construct the nonreduced-fiber equation | none |
...
| Theorem 8.3 (strictly positive boundary SOS) | `sosLength`, `boundary_extreme_support_good_of_ray_dichotomy`, `exists_minimalSquareRepresentation_length_eq_of_fiber_pair_data`, and their supporting lemmas | complete conditional exact-length assembly proved, including `sosLength sq p = m+1`; native finite surjectivity and generic degree `c+2` under flatness are now available | derive flatness and specialize the scheme morphism, continuity/compact source, reduced-fiber locus, and concrete pair bound | none; all remaining geometric inputs are explicit |
...
- `submoduleMultiplicationToDegreeTwo`,
  `symmetricSquare_submoduleMultiplicationToDegreeTwo_surjective`, and
  `hankelKernel_eq_parameterSpan_and_rank_of_degreeTwo_eq_product` (an equality
  `R₁ * R₁ = R₂` transports canonical multiplication and the original functional on `R₂`
  into the complete pointwise parameter kernel/rank theorem).
- `DegreeTwoGeneratedByDegreeOne`, `degreeOne_mul_self_eq_degreeTwo`,
  `symmetricSquare_gradedDegreeOneMultiplication_surjective`, and
  `hankelKernel_eq_parameterSpan_and_rank_of_degreeTwoGeneratedByDegreeOne` (for internal
  homogeneous components, the paper's degree-two generation statement now proves the product
  equality and feeds the rank theorem directly; the equality is no longer an upstream input).
- `ParameterProductGorensteinCertificate.toPerfectPairingCertificate` and
  `hankelKernel_eq_parameterSpan_and_rank_of_gorensteinCertificate` (symmetry proves the
  zero-annihilator and intrinsic perfect-pairing formulations equivalent, so the strongest
  pointwise and graded endpoints accept the weaker Gorenstein certificate directly).
...
- The exact integer formal series `(1+c t+t²)/(1-t)^(m+1)`, its coefficients in degrees
  zero, one, and two, and a graded-component certificate deriving the corresponding finranks.
- Exact cancellation of equation (1) by `(1-t)^(m+1)`, all coefficients of the resulting
  numerator `1+c t+t²`, and `ArtinianReductionHilbertSeriesCertificate`, which derives the
  Artinian Hilbert function `(1,c,1,0,...)` and finite-dimensionality of its degree-two piece.
- `ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate` and
  `hankelKernel_eq_parameterSpan_and_rank_of_artinianReductionHilbertSeriesCertificate`, which
  transport the numerator's degree-two value across a supplied socle equivalence and feed the
  resulting one-dimensional-socle fact into the direct internal-graded kernel/rank endpoint.
- `ArtinianReductionDenominatorRelation.toHilbertSeriesCertificate` and
  `hankelKernel_eq_parameterSpan_and_rank_of_artinianReductionDenominatorRelation`, which narrow
  the numerical reduction input to the exact Hilbert-series quotient identity supplied by the
  linear regular sequence in Proposition 2.2.
- `DegreeOneReductionFinrankRelation.hilbertFunctionSeries_eq_mul_one_sub_X`, its successive-
  reduction iteration theorem, and `hankelKernel_eq_parameterSpan_and_rank_of_successiveLinearReductions`,
  which reduce that global identity to the component-dimension recurrence for each individual
  degree-one nonzerodivisor.
