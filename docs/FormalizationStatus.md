...
| Proposition 2.2 (regular reductions) | `DegreeOneReductionComponentExactSequence.toFinrankRelation`, `DegreeOneReductionFinrankRelation.hilbertFunctionSeries_eq_mul_one_sub_X`, `SuccessiveLinearReductionComponentsRelation.toArtinianReductionDenominatorRelation`, `ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate`, and `Algebra.PerfectPairing` | the finite-dimensional homogeneous short exact sequence for one degree-one nonzerodivisor now mechanically yields its component recurrence; iterating the `(m+1)` recurrences constructs the denominator relation and complete `(1,c,1,0,...)` Hilbert function, whose degree-two value gives the quotient's one-dimensional socle after an equivalence | constructing the concrete graded short exact sequences, the socle equivalence, and zero-annihilator property from the Gorenstein hypotheses remains because mathlib has no Cohen--Macaulay/Gorenstein API | none; explicit interface hypotheses only |
