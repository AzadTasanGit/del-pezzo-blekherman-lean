...
| Proposition 2.2 (regular reductions) | `DegreeOneReductionFinrankRelation.hilbertFunctionSeries_eq_mul_one_sub_X`, `SuccessiveLinearReductionComponentsRelation.toArtinianReductionDenominatorRelation`, `ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate`, and `Algebra.PerfectPairing` | each degree-one nonzerodivisor recurrence multiplies the Hilbert series by `1-t`; iterating the `(m+1)` recurrences constructs the denominator relation and complete `(1,c,1,0,...)` Hilbert function, whose degree-two value gives the quotient's one-dimensional socle after an equivalence | deriving each component recurrence from the actual regular-sequence short exact sequence, the socle equivalence, and zero-annihilator property from the Gorenstein hypotheses remains because mathlib has no Cohen--Macaulay/Gorenstein API | none; explicit interface hypotheses only |
