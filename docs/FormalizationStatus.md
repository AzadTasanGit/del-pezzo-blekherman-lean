...
| Proposition 2.2 (regular reductions) | `DegreeOneReductionComponentExactSequence.toFinrankRelation`, `SuccessiveDegreeOneReductionComponentExactSequences.toFinrankRelations`, `SuccessiveLinearReductionComponentsRelation.toArtinianReductionDenominatorRelation`, `ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate`, and `Algebra.PerfectPairing` | one finite-dimensional homogeneous short exact sequence yields its component recurrence, and an indexed chain whose quotient carriers vary at each stage automatically supplies all successive-reduction data; the resulting iteration constructs the denominator relation and complete `(1,c,1,0,...)` Hilbert function, whose degree-two value gives the quotient's one-dimensional socle after an equivalence | constructing the concrete graded short exact sequences, the socle equivalence, and zero-annihilator property from the Gorenstein hypotheses remains because mathlib has no Cohen--Macaulay/Gorenstein API | none; explicit interface hypotheses only |
