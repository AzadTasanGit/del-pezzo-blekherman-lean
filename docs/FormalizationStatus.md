...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `parameterSpace_finrank_and_hankelRank_of_nonzeroHankel` | compiling with dependency | the actual nonzero induced socle functional now gives the paper's perfect Hankel pairing and rank conclusion without choosing a socle coordinate; the basepoint-free parameter sequence and derivation of the Artinian Gorenstein reduction from the PDF hypotheses remain | none; additional hypotheses are explicit |
