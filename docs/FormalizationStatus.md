...
| Proposition 2.2 (regular reductions) | `DegreeOneReductionComponentExactSequence.toFinrankRelation`, `SuccessiveDegreeOneReductionComponentExactSequences.toArtinianReductionHilbertSeriesCertificateOfInitialEquiv`, `SuccessiveDegreeOneReductionComponentExactSequences.toComponentsRelationOfInitialEquiv`, `hankelKernel_eq_parameterSpan_and_rank_of_successiveDegreeOneReductionExactSequences`, `SuccessiveLinearReductionComponentsRelation.toArtinianReductionDenominatorRelation`, `ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate`, and `Algebra.PerfectPairing` | one finite-dimensional homogeneous short exact sequence yields its component recurrence, and an indexed chain whose quotient carriers vary at each stage now directly constructs the final `(1,c,1,0,...)` Hilbert certificate and feeds the strongest rank endpoint; an explicit degreewise equivalence identifies its initial carrier with the ambient algebra | constructing the concrete graded short exact sequences, initial/socle equivalences, and zero-annihilator property from the Gorenstein hypotheses remains because mathlib has no Cohen--Macaulay/Gorenstein API | none; explicit interface hypotheses only |
| Proposition 3.1 (closed SOS cone and dual) | `SOSConeDual.sosCone_isClosed_of_gram`, `SOSConeDual.sosCone_eq_finiteSumCone`, `finiteSumCone_closed_fullDimensional_dual_of_separating_evaluations`, and supporting Gram results | the named SOS convex cone is identified with finite sums and is closed under the Gram kernel criterion; closedness, convexity, nonempty interior, and exact dual identity are proved from Gram compatibility, degree-two spanning, and separation by real evaluations | instantiating those interfaces from the homogeneous coordinate ring and Zariski-dense real locus remains | none; interface hypotheses are explicit |
