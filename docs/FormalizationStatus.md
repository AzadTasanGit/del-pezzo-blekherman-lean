...
| Proposition 2.2 (regular reductions) | `delPezzoHilbertSeries_mul_one_sub_pow`, `ArtinianReductionDenominatorRelation.toHilbertSeriesCertificate`, `ParameterProductGorensteinCertificate.ofArtinianReductionHilbertSeriesCertificate`, and `Algebra.PerfectPairing` | exact cancellation to the Hilbert numerator and the complete `(1,c,1,0,...)` Hilbert function are proved; the explicit denominator relation constructs the reduction certificate, whose degree-two value constructs the quotient's one-dimensional-socle fact after a socle equivalence | deriving the denominator relation, socle equivalence, and zero-annihilator property from an actual regular sequence and the Gorenstein hypotheses remains because mathlib has no Cohen--Macaulay/Gorenstein API | none; explicit interface hypotheses only |
...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpan_and_rank_of_artinianReductionDenominatorRelation` | compiling with dependency | for an internal multiplicative grading `𝒜`, Lean constructs `R₁ × R₁ → R₂`; surjectivity of `Sym²(R₁) → R₂` proves `R₁ * R₁ = R₂`, while an equation-(1) Hilbert-series certificate derives both finite-dimensionality and `dim R₁=m+c+1`. The strongest endpoint accepts exactly the reduction identity `H_A(t)=H_R(t)(1-t)^(m+1)`, constructs the `(1,c,1)` certificate, and derives the one-dimensional quotient socle after a degree-two equivalence; only the parameter construction, regular-sequence identity, socle equivalence, zero-annihilator proof, and ambient Hilbert certificate must still be obtained from the PDF's basepoint-free AG/CM hypotheses | none; additional hypotheses are explicit |
