...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpan_and_rank_of_parameters_mem_hankelKernel` | compiling with dependency | an explicit linearly independent `(m+1)`-parameter tuple lies pointwise in the Hankel kernel; Lean derives containment of its span, constructs the canonical degree-two relation quotient, uses surjectivity of the PDF's `Sym²(R₁) → R₂` map for Hankel nonzeroness, and derives quotient dimension `c`. The remaining quotient input is the intrinsic one-dimensional perfect multiplication pairing in `ParameterProductPerfectPairingCertificate`, matching Proposition 2.2. Producing this parameter sequence and certificate from the PDF's AG/CM hypotheses remains | none; additional hypotheses are explicit |
