...
|---|---|---|---|---|
| Proposition 2.1 (Hilbert-series consequences) | `delPezzoHilbertSeries`, `coeff_one_delPezzoHilbertSeries`, `coeff_two_delPezzoHilbertSeries`, `DelPezzoHilbertSeriesCertificate.finrank_one`, `DelPezzoHilbertSeriesCertificate.finrank_two`, and the deficiency/degree/`a`-invariant arithmetic in `Algebra.HilbertArithmetic` | exact formal-power-series coefficients and their component-finrank consequences proved | constructing the certificate from a graded ring's Hilbert series, and connecting the numerator to multiplicity/canonical modules, remain; mathlib lacks packaged graded Hilbert-series/canonical-module APIs | none |
...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpan_and_rank_of_hilbertSeriesCertificate` | compiling with dependency | for an internal multiplicative grading `𝒜`, Lean constructs `R₁ × R₁ → R₂`; surjectivity of `Sym²(R₁) → R₂` proves `R₁ * R₁ = R₂`, while an equation-(1) Hilbert-series certificate derives both finite-dimensionality and `dim R₁=m+c+1`. The endpoint combines the original functional on `R₂` with pointwise kernel membership of an independent `(m+1)`-parameter tuple and needs only the one-dimensional-socle/zero-annihilator Gorenstein certificate. Producing the tuple, Gorenstein certificate, and Hilbert certificate from the PDF's basepoint-free AG/CM hypotheses remains | none; additional hypotheses are explicit |
