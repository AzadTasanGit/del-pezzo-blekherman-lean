...
|---|---|---|---|---|
| Proposition 2.1 (Hilbert-series consequences) | `delPezzoHilbertSeries`, its degree-zero through degree-two coefficient theorems, `DelPezzoHilbertSeriesCertificate`, and the deficiency/degree/`a`-invariant arithmetic in `Algebra.HilbertArithmetic` | exact formal-power-series coefficients and their component-finrank consequences proved | constructing the certificate from a graded ring's Hilbert series, and connecting the numerator to multiplicity/canonical modules, remain; mathlib lacks packaged graded Hilbert-series/canonical-module APIs | none |
| Proposition 2.2 (regular reductions) | `IsArithmeticallyGorenstein`, `hankelKernel_eq_parameterSpan_and_rank_of_arithmeticallyGorenstein`, plus the older exact-sequence and perfect-pairing internals | the paper-facing predicate names the actual degree-one regular sequence, its Artinian ideal quotient, the one-dimensional degree-two socle, and perfect degree-one multiplication; with the literal Hilbert equation it gives the actual Hankel kernel and rank without exposing any old algebra certificate | instantiate this operational predicate for the coordinate ring; the finite-Proj/degree endpoint still requires removal of its free-basis certificate | none; explicit mathematical hypotheses only |
