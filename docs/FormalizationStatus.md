...
| Proposition 2.2 (regular reductions) | `IsArithmeticallyGorenstein`, `hankelKernel_eq_parameterSpan_and_rank_of_arithmeticallyGorenstein`, `degreeOneParameterIdeal_quotient_finrank`, and `genericRank_eq_add_two` | the paper-facing predicate names the actual regular sequence and perfect socle multiplication; the concrete quotient sequences give length `c+2`, the Hankel theorem gives rank `c`, and local lifting from regularity gives generic rank `c+2`, all without old algebra certificates, flatness, or assumed freeness | instantiate the operational predicate and select the regular hsop from the native coordinate-ring/kernel hypotheses | none; no freeness hypothesis remains in the strongest generic-rank theorem |
...
| Proposition 5.1 | `vanishes_on_ker_iff_eq_smul`, `exists_projective_evaluation_omitting_hyperplane`, `projectiveAevalOfRadical_isFinite_surjective_of_standardGraded_hsop`, `genericRank_eq_add_two`, `dense_compl_of_subset_mvPolynomial_zeroSet`, `surjective_of_dense_fiber_pair_bounds` | common-zero/projective-fiber linear algebra, certificate-free finite surjectivity, generic degree `c+2` from regular parameters, density, compact-image reduction, and perturbation logic proved | connect the native kernel/regular hsop to the scheme morphism, realize continuity, and construct the nonreduced-fiber equation | none |
...
| Theorem 8.3 (strictly positive boundary SOS) | `sosLength`, `boundary_extreme_support_good_of_ray_dichotomy`, `exists_minimalSquareRepresentation_length_eq_of_fiber_pair_data`, and their supporting lemmas | complete conditional exact-length assembly proved, including `sosLength sq p = m+1`; native finite surjectivity and generic degree `c+2` from regular parameters are now available | specialize the scheme morphism, continuity/compact source, reduced-fiber locus, and concrete pair bound | none; all remaining geometric inputs are explicit |
