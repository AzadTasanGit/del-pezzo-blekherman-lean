...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpace_and_rank_of_linearIndependentParameters` | compiling with dependency | the degree-two relation space is canonical, radical containment descends the functional, surjectivity of the PDF's `Sym²(R₁) → R₂` multiplication gives Hankel nonzeroness, and an explicit linearly independent `(m+1)`-parameter tuple supplies the parameter dimension and hence quotient dimension `c`. The remaining quotient input is the intrinsic one-dimensional perfect multiplication pairing in `ParameterProductPerfectPairingCertificate`, matching Proposition 2.2. Producing the parameter sequence and this certificate from the PDF's AG/CM hypotheses remains | none; additional hypotheses are explicit |
