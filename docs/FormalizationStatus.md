...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpace_and_finrank_and_rank_of_factorization` | compiling with dependency | the ambient Hankel form is now identified with the quotient multiplication pullback and has its actual kernel, kernel dimension, and rank computed; constructing the basepoint-free parameter sequence and the quotient multiplication/socle data from the PDF's AG/CM hypotheses remains | none; additional hypotheses are explicit |
