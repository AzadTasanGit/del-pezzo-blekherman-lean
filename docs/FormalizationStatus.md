...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpan_and_rank_of_degreeTwo_eq_product` | compiling with dependency | for named degree pieces `U=R₁` and `Q=R₂`, an explicit equality `U * U = Q` transports canonical ring multiplication to the original degree-two target; Lean proves symmetry and symmetric-square surjectivity internally and combines these with pointwise kernel membership of an independent `(m+1)`-parameter tuple. The remaining quotient input is the intrinsic perfect pairing certificate matching Proposition 2.2; proving `R₁ * R₁ = R₂` and producing the parameters/certificate from the PDF's standard-graded AG/CM hypotheses remain | none; additional hypotheses are explicit |
