...
| Lemmas 4.1--4.2 and Theorem 4.3 | `extreme_psd_evaluation_or_basepointFree`, `kernel_dimension_and_rank_from_perfect_quotient`, `hankelKernel_eq_parameterSpan_and_rank_of_degreeTwoGeneratedByDegreeOne` | compiling with dependency | for an internal multiplicative grading `𝒜`, Lean constructs `R₁ × R₁ → R₂`; the PDF's literal degree-two consequence of standard gradedness, surjectivity of `Sym²(R₁) → R₂`, proves `R₁ * R₁ = R₂` via the internally equivalent tensor map. The endpoint combines the original functional on `R₂` with pointwise kernel membership of an independent `(m+1)`-parameter tuple and needs only the one-dimensional-socle/zero-annihilator Gorenstein certificate; Lean proves this equivalent to the intrinsic perfect pairing. Producing the tuple and certificate from the PDF's basepoint-free AG/CM hypotheses remains | none; additional hypotheses are explicit |
