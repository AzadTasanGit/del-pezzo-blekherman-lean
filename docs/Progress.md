...
## 2026-08-20: concrete regular quotient and basis-free degree

- Constructed the additive decomposition of an actual homogeneous ideal quotient into its
  degreewise quotient components and proved the required finite direct-sum finrank formula.
- Proved the reverse kernel-to-range inclusion for adjoining a regular degree-one parameter.
  The resulting concrete short exact sequence removes the supplied successive-reduction
  certificate from the numerical argument.
- Iterated those concrete recurrences along the actual parameter prefix ideals and derived the
  Artinian quotient Hilbert function `(1,c,1,0,...)` directly from the literal Hilbert series.
- Proved `degreeOneParameterIdeal_quotient_finrank`, giving total quotient dimension `c+2`.
- Added the basis-free origin-fiber bridge and proved the missing local algebra lemma:
  `Module.free_of_isRegular_of_span_eq_maximalIdeal` lifts freeness backwards through a regular
  sequence, and `degreeOneParameter_free_at_origin_of_isRegular` applies it at parameter-space
  origin.
- `genericRank_eq_add_two` now proves generic degree `c+2` from regularity, the literal Hilbert
  equation, and module finiteness, with no global basis, flatness, assumed local freeness, or
  project-specific algebra certificate. The older local-free and flat endpoints remain wrappers.
