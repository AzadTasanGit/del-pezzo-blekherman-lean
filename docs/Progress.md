...
- Added the basis-free origin-fiber bridge: for a finite flat parameter extension, generic rank
  equals the actual parameter-quotient dimension, hence `genericRank_eq_add_two_of_flat` proves
  generic degree `c+2` without a global basis or any project-specific algebra certificate.
- Isolated the remaining algebraic input to the standard implication that the paper's
  Cohen--Macaulay coordinate ring is flat over the polynomial ring generated by a regular hsop.
