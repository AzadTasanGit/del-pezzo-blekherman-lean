...
| 3. `dim W_ell=m+1`, `rank Q_ell=c` | conditional theorem proved | derive Hilbert dimensions and socle-annihilator property from the PDF's AG/CM hypotheses |
...
- Forty-eight source modules plus the axiom audit, totaling 6,161 Lean source lines.
- No `sorry`, `admit`, custom `axiom`, `TODO`, `FIXME`, or `#check` in any staged Lean source.
- The declarations listed by `AxiomCheck.lean` depend only on `propext`, `Classical.choice`, and
  `Quot.sound`.
- Every dependency was recompiled to a fresh `.olean` before the final check.
- Forced-fresh compilation of all forty-eight modules and `AxiomCheck.lean` exited zero.
